...
export const createGurukulArtRequest = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (!localSuccess && !cloudSuccess) {
            return res.status(500).json({ success: false, message: 'ડેટાબેઝ ઇન્સર્ટ ફેલ થયો છે.' });
        }
        return res.status(201).json({ success: true, message: `Gurukul Art એપ્લિકેશન સફળતાપૂર્વક સબમિટ થઈ ગઈ છે! ⏳`, data: newRequest });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Internal server error' });
    }
};

export const getGurukulArtRequests = async (req: Request, res: Response): Promise<any> => {
    try {
        const selectQuery = `
            SELECT id, name, suid, performance, description, department_id, image_url, status, is_user_created, created_at
            FROM admit_requests WHERE department_id = 2 ORDER BY id DESC;
        `;
        let requestsList = [];
        ...
        return res.status(500).json({ success: false, message: 'Error fetching Gurukul Art requests' });
    }
};
...
export const approveGurukulArtRequest = async (req: Request, res: Response): Promise<any> => {
    try {
        const { id } = req.params;
        ...
        if (!isSuperAdmin && (!isHead || Number(departmentId) !== 2)) {
...
export const declineGurukulArtRequest = async (req: Request, res: Response): Promise<any> => {
    try {
        const { id } = req.params;
        const { adminId, userRole, departmentId } = req.body;
        const isSuperAdmin = adminId === "123098" || Number(adminId) === 123098 || String(userRole).toUpperCase() === "SUPER_ADMIN";
