...
export const createGMusicRequest = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (!localSuccess && !cloudSuccess) {
            return res.status(500).json({ success: false, message: 'ಡેટಾબેઝ ઇન્સર્ટ ફેલ થયો છે.' });
        }
        return res.status(201).json({ success: true, message: `G-Music એપ્લિકેશન સફળતાપૂર્વક સબમિટ થઈ ગઈ છે! ⏳`, data: newRequest });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Internal server error' });
    }
};

export const getGMusicRequests = async (req: Request, res: Response): Promise<any> => {
    try {
        const selectQuery = `
...
export const getOnboardedGMusicUsers = async (req: Request, res: Response): Promise<any> => {
    try {
        const query = `
...
export const approveGMusicRequest = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const isSuperAdmin = adminId === "123098" || Number(adminId) === 123098 || String(userRole).toUpperCase() === "SUPER_ADMIN";
        const isHead = String(userRole).toUpperCase() === "HEAD1029" || String(userRole).toUpperCase() === "DEPARTMENT MAIN";
        ...
        const updateQuery = `UPDATE admit_requests SET status = 'Approved' WHERE id = $1 AND department_id = 1 RETURNING id, name, status;`;
        let updatedRow = null;
        ...
        return res.status(200).json({ success: true, message: `G-Music Request Approved! ✅`, data: updatedRow });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Server error' });
    }
};
...
export const declineGMusicRequest = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const updateQuery = `UPDATE admit_requests SET status = 'Declined' WHERE id = $1 AND department_id = 1 RETURNING id, name, status;`;
        let updatedRow = null;
        try { updatedRow = (await pool.query(updateQuery, [id])).rows[0]; } 
        catch (err) { updatedRow = (await neonPool.query(updateQuery, [id])).rows[0]; }
        if (!updatedRow) return res.status(404).json({ success: false, message: "રિક્વેસ્ટ મળી નથી." });
        return res.status(200).json({ success: true, message: `G-Music Request Declined! ❌`, data: updatedRow });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Server error' });
    }
};